use crate::config;
use serenity::all::{CommandInteraction, CommandOptionType, CreateCommand, CreateCommandOption, ResolvedOption, ResolvedValue};
    ...
    if let Some(ResolvedOption {
        ...
        return if let Some(guild_id) = interaction.guild_id {
            ...
            if let Err(error) = config::set_guild_alert_channel(guild_id, Some(channel.id)) {
                ...
                channel.name.as_ref().unwrap(), interaction.user.name
            );
                format!("Set alert channel to: {}", channel.name.as_ref().expect("No name?"))
            }
        } else {
        ...
        return if let Some(guild_id) = interaction.guild_id {
            if let Err(err) = config::set_guild_alert_channel(guild_id, None) {
                ...
                tracing::info!("Removed alerts channel at request of {}", interaction.user.name);
